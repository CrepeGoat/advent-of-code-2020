...
fn parse_input<R: BufRead>(reader: R) -> HashSet<Coords> {
	reader.lines()
		...
		.flat_map(|(i, s)|
			s.chars().enumerate()
				.filter(|(_j, ch)| *ch == '#')
				.map(move |(j, _ch)| (i as isize, j as isize, 0isize, 0isize))
...
fn neighbors(coords: &Coords) -> impl Iterator<Item=Coords> {
	...
	let l_vals = [coords.3, coords.3-1, coords.3+1];

	let iter = itertools::iproduct!(
		i_vals.iter(), j_vals.iter(), k_vals.iter(), l_vals.iter()
	).skip(1);  // discard center of cube
	iter.map(|(&i, &j, &k, &l)| (i, j, k, l)).collect::<Vec<_>>().into_iter()
}
