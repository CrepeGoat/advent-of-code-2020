...
def simulate_seats(array):
    mask_not_seats = (array == 2)
    while True:
        yield array
        array[mask_not_seats] = 0
        array = (scipy.signal.convolve2d(
            array,
            np.array([[-1, -1, -1], [-1, 3, -1], [-1, -1, -1]]),
            mode='same', boundary='fill',
        ) >= 0).astype(np.int8)
        array[mask_not_seats] == 2
...
print(np.count_nonzero(array2 == 1))
